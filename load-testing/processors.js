...
function setReservationTimeSlot(context, events, done) {
    const randomIndex = Math.floor(Math.random() * context.vars.resources.length);
...
function getRandomUserToken(context, events, done) {
    const randomIndex = Math.floor(Math.random() * data.length);//context.vars.resources.length
